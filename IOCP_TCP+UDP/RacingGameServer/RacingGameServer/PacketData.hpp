...
public:
	...
	/// 버퍼 메모리를 할당합니다.
	/// 기존 메모리가 있었다면 재할당되며
	/// 버퍼에 적혀있던 데이터는 가능한 크기만 복사됩니다.
	/// 할당에 실패하면 false를 반환합니다.
	/// </summary>
	...
	/// <summary>
	/// 버퍼에 적힌 데이터 일부를 제거합니다.
	/// 이후 남은 데이터는 메모리 상단에 맞춥니다.
	/// </summary>
	/// <param name="size_"></param>
	/// <returns></returns>
	void Pop(uint32_t size_);
	...
	/// 
	/// POD객체에 사용해주세요.
	/// (서버에서 송신할 데이터에 대해 UDP/TCP 구분 없이 사용합니다.)
	/// UDP헤더를 구분해서 사용한 이유는 서버측에서 클라이언트를 구분할 토큰이 필요해서 였고,
	/// 클라이언트는 토큰을 구분할 필요는 없습니다.
	/// </summary>
		...
		Init(InfoCode infoCode_, uint32_t reqNo_, T& objData_)
	{
		...
		if (size > 0)
		{
			CopyMemory(m_pData + m_Size, &objData_, sizeof(T));
			m_Size += sizeof(T);
		}
